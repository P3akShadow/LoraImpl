...
class FinetuneRoberta(RobertaSelfAttention):
    def __init__(self, alpha=1.0, *args, **kwargs):
        ...
        self.ft_query = nn.Parameter(torch.zeros(dimension, dimension))
        self.ft_value = nn.Parameter(torch.zeros(dimension, dimension))
    ...
    def forward(
        ...
        """Modified forward pass using finetuning for query and value computations"""
...
class FinetuneWrapperRoberta(nn.Module):
    def __init__(self, task_type, num_classes: int = None, dropout_rate=0.1, model_id: str = "roberta-base",
        ...
        """
        Initializes a FinetuneWrapperRoberta instance, which is a wrapper around the RoBERTa model incorporating
        Finetuning to retrain the model for different NLP tasks such as GLUE benchmarks
        and SQuAD.
        ...
        self.freeze_parameters_except_finetune_and_bias()

    def replace_multihead_attention(self, verbose=True):
        """
        Replaces RobertaSelfAttention with FinetuneRobertaSelfAttention in the model, which contains the finetune logic and parameters.
        ...
        if verbose:
            print(f"Replaced {self.nr_replaced_modules} modules of RobertaSelfAttention with FinetuneRobertaSelfAttention")
    ...
    def freeze_parameters_except_finetune_and_bias(self):
        ...
        for name, param in self.model.named_parameters():
            if ("ft_" in name) or ("finetune_head_" in name) or (self.train_biases and "bias" in name) \
